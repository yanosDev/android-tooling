...
/**
 * Modal dialog presenting a scroll-wheel date picker (day, month, year).
 *
 * Maintains its own internal state seeded from [initialValue]. Day values are automatically
 * clamped when the selected month or year changes (e.g. 31 Jan → 28/29 Feb).
 *
 * @param onConfirm Called with the selected [YDPickerDate] when the confirm button is tapped.
 ...
 * @param initialValue Date shown when the dialog first opens.
 ...
 */
@Composable
fun YDDatePickerDialog(
    onConfirm: (YDPickerDate) -> Unit,
    onDismissRequest: () -> Unit,
    modifier: Modifier = Modifier,
    confirmText: String = "OK",
    dismissText: String? = null,
    initialValue: YDPickerDate = YDPickerDate(day = 1, month = 1, year = 2026),
...
private fun Preview() = YDPreview {
    YDDatePicker(
        value = YDPickerDate(day = 15, month = 4, year = 2026),
        onValueChange = {},
    )
}
