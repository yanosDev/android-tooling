...
import de.yanosdev.styleguide.R.style.Theme_YD_EdgeToEdgeFloatingDialogWindow
...
internal fun YDBottomSheetDialog(
    ...
    val dialog = remember {
        YDBottomSheetDialogWrapper(
            ...
            setContent(parentComposition = composition) {
                Box(modifier = Modifier.semantics { dialog() }) {
                    currentContent()
                }
            }
        }
...
internal class YDBottomSheetDialogWrapper(
    ...
    ContextThemeWrapper(
        composeView.context,
        Theme_YD_EdgeToEdgeFloatingDialogWindow
    )
), ViewRootForInspector {
    private val dialogLayout: YDBottomSheetDialogLayout
    ...
    init {
        ...
        dialogLayout = YDBottomSheetDialogLayout(
            context = context,
            window = window,
            ...
            // case of shapes like circle the area for dismiss might be to small (rectangular
            // outline consuming clicks outside the circle).
            outlineProvider = object : ViewOutlineProvider() {
                override fun getOutline(view: View, result: Outline) {
                    result.setRect(0, 0, view.width, view.height)
                    // We set alpha to 0 to hide the view's shadow and let the
                    // composable to draw its own shadow. This still enables us to get the extra
                    // space needed in the surface.
                    result.alpha = 0f
                }
            }
        }
    ...
    private fun setSecurePolicy(securePolicy: SecureFlagPolicy) {
        val secureFlagEnabled = securePolicy.shouldApplySecureFlag(
            isSecureFlagSetOnParent = composeView.isFlagSecureEnabled()
        )
        window!!.setFlags(
...
private class YDBottomSheetDialogLayout(
    ...
    private val onBackEvent: (YDBottomSheetBackEvent) -> Unit,
    ...
    private object Api34Impl {
        ...
        fun createBackCallback(
            ...
            override fun onBackStarted(backEvent: BackEvent) {
                onBackEvent(YDBottomSheetBackEvent.OnBackStarted(backEvent))
            }

            override fun onBackProgressed(backEvent: BackEvent) {
                onBackEvent(YDBottomSheetBackEvent.OnBackProgressed(backEvent))
            }

            override fun onBackInvoked() {
                onBackEvent(YDBottomSheetBackEvent.OnBackInvoked)
                onDismissRequest()
            }

            override fun onBackCancelled() {
                onBackEvent(YDBottomSheetBackEvent.OnBackCancelled)
            }
        }
    }
    ...
    private object Api33Impl {
        ...
        fun createBackCallback(
            onDismissRequest: () -> Unit,
        ) = OnBackInvokedCallback(function = onDismissRequest)
