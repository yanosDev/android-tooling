...
/**
 * Static outlined card with a visible border and no interaction.
 *
 * Defaults to [YDCardDefaults.outlinedCardColors], which uses [de.yanosdev.styleguide.theme.foundations.semantics.YDColors.line] as the border.
 *
 * @param modifier Modifier applied to the card surface.
 * @param colors Color scheme controlling border, background, and content colors.
...
fun YDOutlinedCard(
    ...
    colors: YDCardColors = YDCardDefaults.outlinedCardColors(),
...
/**
 * Clickable outlined card with ripple feedback and a visible border.
 *
    ...
    Column(modifier = Modifier.padding(all = spacings.medium)) {
        YDOutlinedCard(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(all = spacings.large)) {
                YDText(text = "Outlined card", style = typography.h5)
...
private fun ClickablePreview() = YDPreview {
    Column(modifier = Modifier.padding(all = spacings.medium)) {
        YDOutlinedCard(onClick = {}, modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(all = spacings.large)) {
                YDText(text = "Clickable outlined card", style = typography.h5)
